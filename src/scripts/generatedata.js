...
const guitarsDir = path.join(projectRoot, 'public', 'vetement_femme')
...
function cleanNameFromFilename(filename) {
  ...
  if (/photo/i.test(filename)) return 'Vêtements femme'
  if (/img/i.test(filename)) return 'Vêtements femme'
...
function buildProduct(id, dir, files) {
  ...
  let minOrder = rand(5, 15)
  if (price < 1500) minOrder = rand(5, 13)
...
function generate() {
  if (!fs.existsSync(guitarsDir)) {
    console.error('vetement_femme dir not found:', guitarsDir)
    process.exit(1)
  }

...
import {
  ...
  Zap, Crown, Sparkles, LayoutGrid, Watch, Gem, Link2, Circle
} from 'lucide-react';
import SubCategoryNav from '../Components/SubCategoryNav';

const BIJOU_ICONS = {
  all:              <LayoutGrid className="w-4 h-4" />,
  montres:          <Watch      className="w-4 h-4" />,
  colliers:         <Gem        className="w-4 h-4" />,
  boucles_oreilles: <Sparkles   className="w-4 h-4" />,
  bracelets:        <Link2      className="w-4 h-4" />,
  bagues:           <Circle     className="w-4 h-4" />,
  lunettes:         <Eye        className="w-4 h-4" />,
};
...
const BijouCategoryPage = () => {
  ...
  const handleProductClick = (product) => {
    navigate(`/product/bijoux/${product.sourceId}`);
  };

  // Données de la catégorie Bijoux
  const categoryData = {
    id: 'jewelry',
    name: 'Bijoux & Montres',
    icon: '💎',
    description: 'Découvrez notre collection élégante de bijoux et montres de luxe directement des meilleurs fournisseurs chinois. Qualité premium, prix d\'usine.',
    bannerImage: 'https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?auto=format&fit=crop&w=2070&q=80',
    features: [
  ...
  const computedSubIds = ['all','montres','colliers','boucles_oreilles','bracelets','bagues','lunettes']
  ...
  // Filtres avancés pour bijoux
  const sizes = ['Unique', '38mm', '40mm', '42mm', '44mm', '46mm', '40cm', '45cm', '50cm', '55cm', '60cm', '15cm', '17cm', '19cm', '21cm'];
  // dériver les couleurs et marques depuis les données
  const colorPool = {
    'Or': '#FFD700', 'Argent': '#C0C0C0', 'Rose Gold': '#B76E79', 'Noir': '#000000', 
    'Blanc': '#FFFFFF', 'Bleu': '#3B82F6', 'Rouge': '#EF4444', 'Vert': '#10B981', 
    'Gris': '#6B7280', 'Argenté': '#D3D3D3'
  }

  const colors = Object.keys(colorPool).map(name => ({ name, value: colorPool[name] }))

  // Utiliser les produits de bijoux et normaliser les champs attendus par l'UI
  ...
  const normalizeProduct = (product) => {
    ...
    return {
      ...
      brandId,
      brandName,
      supplier: product.supplier || 'Fournisseur Premium',
      features: product.features || ['Bijou de qualité premium', 'Finition élégante'],
      rating: product.rating || +(4.2 + Math.random() * 0.8).toFixed(1),
      reviews: product.reviews || rand(10, 300),
      stock: product.stock || rand(50, 500),
      sold: product.sold || rand(20, 400),
  ...
  const products = bijoux_accessoires.map(p => normalizeProduct(p))
  ...
  const subCategories = computedSubIds.map(id => {
    ...
    return {
      id,
      name: id === 'all' ? 'Tous les bijoux' : (
        id === 'montres' ? 'Montres' :
        id === 'colliers' ? 'Colliers' :
        id === 'boucles_oreilles' ? 'Boucles d\'oreilles' :
        id === 'bracelets' ? 'Bracelets' :
        id === 'bagues' ? 'Bagues' :
        id === 'lunettes' ? 'Lunettes' : id
      ),
      count,
...
export default BijouCategoryPage;
...
import {
  ...
  Zap, Crown, Sparkles, LayoutGrid, Baby, Package, ShoppingBag, Plane
} from 'lucide-react';
import SubCategoryNav from '../Components/SubCategoryNav';

const BAG_ICONS = {
  all:              <LayoutGrid  className="w-4 h-4" />,
  sacs:             <Package     className="w-4 h-4" />,
  femme:            <ShoppingBag className="w-4 h-4" />,
  enfant:           <Baby        className="w-4 h-4" />,
  sacs_voyage:      <Plane       className="w-4 h-4" />,
  sacs_bandouliere: <Package     className="w-4 h-4" />,
};
import { sacs } from '../data/sacs';
import { sac } from '../data/sac_femme';
...
const BagCategorypage = () => {
  ...
  const handleProductClick = (product) => {
    navigate(`/product/sacs/${product.sourceId}`);
  };
...
const categoryData = {
  ...
  icon: '👜',
  description: 'Découvrez notre collection moderne de sacs à main, sacs à dos et accessoires de maroquinerie. Des modèles tendance, qualité premium et prix imbattables directement depuis les meilleurs fournisseurs.',
  bannerImage: 'https://images.unsplash.com/photo-1522336572468-97b06e8ef143?auto=format&fit=crop&w=2070&q=80',
  features: [
    { icon: <Shield className="w-6 h-6" />, text: 'Qualité supérieure' },
    { icon: <Truck className="w-6 h-6" />, text: 'Livraison internationale' },
    { icon: <Clock className="w-6 h-6" />, text: 'Disponibilité 24/7' },
    { icon: <Users className="w-6 h-6" />, text: 'Fournisseurs certifiés' }
  ]
};


  const computedSubIds = ['all','sacs','femme','enfant','sacs_voyage','sacs_bandouliere','dresses','sportswear','suits','jackets']
  ...
  const products = [
    ...sacs.map(p => normalizeProduct(p, 'sacs')),
    ...sac.map(p => normalizeProduct(p, 'femme')),
  ]
  ...
  const subCategories = computedSubIds.map(id => {
    ...
    return {
      id,
      name: id === 'all' ? 'Tous sacs' : (
        id === 'sacs' ? 'sac a dos' :
        id === 'femme' ? 'sac a main femme' :
        id === 'enfant' ? 'sac a dos enfant' :
        id === 'sacs_voyage' ? 'sac de voyage' :
        id === 'pants' ? 'bandouliere' :
  ...
  const filteredProducts = products.filter(product => {
    const matchesSub = (() => {
      ...
      if (activeSubCategory === 'sacs') return product.gender === 'sacs'
      if (activeSubCategory === 'femme') return product.gender === 'femme'
                        ...
                        <div className="relative mb-2 sm:mb-3 md:mb-4">
                          <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden">
                            <img
                              src={product.image}
                              alt={product.name}
                              onClick={() => navigate('/produit', { state: { productId: product.sourceId, category: 'bags' } })}
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          <div>
                            <h3 className="font-medium text-gray-800 text-sm leading-5 line-clamp-2 mb-1 h-10 overflow-hidden">
                              <span onClick={() => navigate('/produit', { state: { productId: product.sourceId, category: 'bags' } })} className="cursor-pointer">{getProductName(product)}</span>
                            </h3>
                          </div>
                          {/* Prix */}
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        onClick={() => {
                          ...
                          navigate('/produit', { state: { productId: quickViewProduct.sourceId, category: 'bags' } })
                        }}
...
export default BagCategorypage;